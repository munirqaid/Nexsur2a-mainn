...
// إنشاء منشور جديد (يدعم النصوص والوسائط)
router.post('/', authenticateToken, upload.single('media'), async (req, res) => {
  try {
    const { content, postType, location, hashtags, mentions, isMonetized } = req.body;
    ...
    let mediaUrls = [];
    if (req.file) {
      // تحويل المسار ليكون متاحاً عبر الويب
      const fileUrl = `/uploads/${req.file.filename}`;
      mediaUrls.push(fileUrl);
    }

    // التأكد من أن المحتوى ليس فارغاً لأن المخطط يتطلب وجوده (required: true)
    const postContent = content || (req.file ? 'صورة' : '');

    const newPost = new Post({
      userId,
      content: postContent,
      postType: req.file ? 'image' : (postType || 'text'),
      mediaUrls: mediaUrls,
